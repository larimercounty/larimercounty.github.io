var app = angular.module("roadEvents", ['tableSort','ngDialog','ngResource']);
...
app.controller("EventsController", function( $scope,$filter,$resource,ngDialog,$location,$anchorScroll,$timeout) {
    ...
    // I load the remote data.
     function getlinedata($scope) {
        ...
        "https://agomaps.larimer.org/arcgis/rest/services/test/RoadAndBridgeEventConsumer/MapServer/1/query?where=OBJECTID+%3E+0&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=*&returnGeometry=true&returnTrueCurves=false&maxAllowableOffset=&geometryPrecision=&outSR=4326&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&returnDistinctValues=false&resultOffset=&resultRecordCount=&f=pjson",
           	...
           	if(  document.addEventListener ){ //check to see if IE greater than IE8
           		linedata.forEach(function(event) {
           			//var event=linedata[i];
            			...
            			event.url = 'http://larimercounty.maps.arcgis.com/apps/Embed/index.html?webmap=1ae8ab4813ba440cbf03565258ac4e86&extent='+extent;
    ...
    function getpointdata($scope) {
    	var ArcGISAPIPoints = $resource(
        	"https://agomaps.larimer.org/arcgis/rest/services/Test/RoadAndBridgeEventConsumer/MapServer/0/query?where=OBJECTID+%3E+0&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&relationParam=&outFields=*&returnGeometry=true&returnTrueCurves=false&maxAllowableOffset=&geometryPrecision=&outSR=4326&returnIdsOnly=false&returnCountOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&gdbVersion=&returnDistinctValues=false&resultOffset=&resultRecordCount=&f=pjson",
        	{callback: "JSON_CALLBACK"},
        	{get: {method: "JSONP"}});
        	...
        	function( pointevents ) {
           	...
           	if(  document.addEventListener){   //check to see if IE greater than IE8
           		pointdata.forEach(function(event) {
            		...
            		event.url = 'http://larimercounty.maps.arcgis.com/apps/Embed/index.html?webmap=1ae8ab4813ba440cbf03565258ac4e86&center='+center.lon+','+center.lat+'&level=16';
                if($scope.event_id == event.attributes.OBJECTID) {
                    var startdisplay =$filter('date')(event.attributes.STARTDATE, 'MM/dd/yyyy');
                    var enddisplay =$filter('date')(event.attributes.ENDDATE, 'MM/dd/yyyy');
    ...
    function getCoordinates(y, x) {   
           ...
           */
            ...
            retObj.lat = y.toFixed(6); //lat.toFixed(6);
            retObj.lon = x.toFixed(6); //lon.toFixed(6);
            return retObj;
        } 
}); 
...
app.filter('prepimagename', function () {
        return function (text) {
